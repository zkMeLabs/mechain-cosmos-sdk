...
func MakeTestEncodingConfig(mb module.BasicManager) TestEncodingConfig {
	...
	sdk.RegisterLegacyAminoCodec(encCfg.Amino)

	keyring.RegisterLegacyAminoCodec(encCfg.Amino)
	cryptocodec.RegisterCrypto(encCfg.Amino)
	legacy.Cdc = encCfg.Amino
	codec.RegisterEvidences(encCfg.Amino)
...
func SetEncodingConfig(cfg TestEncodingConfig) {
	eip712.AminoCodec = cfg.Amino
	eip712.ProtoCodec = codec.NewProtoCodec(cfg.InterfaceRegistry)
}

func (suite *EIP712TestSuite) SetupTest() {
	suite.config = MakeTestEncodingConfig(simapp.ModuleBasics)
	suite.clientCtx = client.Context{}.WithTxConfig(suite.config.TxConfig)
	suite.denom = BaseDenom

	sdk.GetConfig().SetBech32PrefixForAccount(Bech32Prefix, "")
	SetEncodingConfig(suite.config)
}
		...
		for _, signMode := range signModes {
			suite.Run(tc.title, func() {
				...
				signerData := authsigning.SignerData{
					...
					Sequence:      params.sequence,
					PubKey:        pubKey,
					Address:       sdk.MustBech32ifyAddressBytes(Bech32Prefix, pubKey.Bytes()),
				}
